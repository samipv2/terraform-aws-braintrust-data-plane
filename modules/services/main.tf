data "aws_region" "current" {
  lifecycle {
    postcondition {
      ...
      error_message = "Region must be one of: us-east-1, us-east-2, us-west-2. Contact support if you need this region to be supported."
    }
  }
}
...
resource "aws_lambda_function" "ai_proxy_fn" {
...
resource "aws_lambda_function_url" "ai_proxy_fn" {
  function_name      = aws_lambda_function.ai_proxy_fn.function_name
...
resource "aws_lambda_alias" "ai_proxy_fn_live" {
  name             = "live"
  function_name    = aws_lambda_function.ai_proxy_fn.function_name
  function_version = aws_lambda_function.ai_proxy_fn.version
}

resource "aws_lambda_permission" "ai_proxy_fn" {
  statement_id = "AllowFunctionURLInvoke"
  action       = "lambda:InvokeFunctionUrl"

  function_name          = aws_lambda_function.ai_proxy_fn.function_name
  qualifier              = aws_lambda_alias.ai_proxy_fn_live.name
  principal              = "*"
